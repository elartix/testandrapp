...
import com.codegineer.datagrid.DataGridStyle.HeaderContainer;
import com.codegineer.datagrid.DataGridStyle.DataContainer;
import com.codegineer.datagrid.DataGridStyle.HeaderSpliterCell;
...
public class DataGrid extends LinearLayout{
	...
	private void initHeader()
	{
		...
		for(int i = 0; i < mc.COLUMN_STYLE.size(); i++)
		{
			...
			mListHeader.addView(HeaderView,new LayoutParams(LayoutParams.WRAP_CONTENT, HeaderCell.Height));
						
			if(intCellSpliter < mc.COLUMN_STYLE.size())
			{
				Spliter = new Splitter(getContext(), mc, i);
				Spliter.setIndex(i);
				mListHeader.addView(Spliter,new LayoutParams(HeaderCell.Width, HeaderCell.Height));
				intCellSpliter++;
			}
		}
	}
